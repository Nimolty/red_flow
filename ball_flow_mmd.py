...
from Envs.RLEnvSortingBall_new_2 import RLSorting
from Envs.mmd_loss import mmd, gaussian_kernel
...
def _log_prob(flow, inputs):
    ...
    log_prob = flow._distribution.log_prob(noise)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', type = str, default='mmd_loss_0424')
    ...
    num_layers = 5
    #base_dist = BoxUniform(torch.tensor([-1.0, -1.0]*num_boxes).to(device), torch.tensor([1.0, 1.0] * num_boxes).to(device))
    ...
    #origin_sample = samples[:sample_num].to(device)
    #z_sample = torch.randn((sample_num, 2 * num_boxes)).to(device)
    ...
    #transforms.append(Tanh())
    transform = CompositeTransform(transforms)
    flow = Flow(transform, base_dist).to(device)
    optimizer = optim.Adam(flow.parameters())
    #print(base_dist.cuda())
    ...
    for epoch in trange(epoches):
        ...
        if epoch <= 10:
            ...
            for i, sample in enumerate(loader):
                ...
                optimizer.zero_grad()
                loss = -_log_prob(flow, sample).mean()
                index = np.random.choice(np.arange(1e5), sample_num)
                origin_sample = sample[index].to(device)
                states, _ = flow._transform.inverse(z_sample).to(device)
                loss += mmd(origin_sample, states)
                print(loss)
                loss.backward()
                optimizer.step()
                writer.add_scalar('Loss', loss, i + epoch * len(loader))
        ...
        #break
        with torch.no_grad():
            states = flow.sample(16)
            # noise_z = flow._distribution.rsample(torch.Size([1,16])).to(device)
            # states, _ = flow._transform.inverse(noise_z[0])
            states = states.clone().to(torch.float64)
            print(states.max(), states.min())     #print(states)
